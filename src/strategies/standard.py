...
class StandardStrategy(BaseStrategy):
    ...
    def record_pattern(self) -> re.Pattern[str]:
        ...
        return rf"(\d+\. )(?P<title>(.|\n){{,250}}?)(?P<measure_unit>\s.{{,5}}?)(?P<quantity>\s\d+(.\d+)?)(?P<unit_price>{number_pattern2})(?P<total_price_no_vat>{number_pattern})(?P<vat_rate>\s\d+)(?P<total_vat>{number_pattern})(?P<total_price_vat>{number_pattern})"  # noqa: E501
