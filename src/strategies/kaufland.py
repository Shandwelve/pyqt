...
class KauflandStrategy(BaseStrategy):
    ...
    def record_pattern(self) -> re.Pattern[str]:
        ...
        return rf"(\d+\. )?(?P<title>(.)+?)(?P<measure_unit>\s.{{,5}}?)(?P<quantity>\s\d+(,\d+)?).?(?P<unit_price>{number_pattern}).?(?P<total_price_no_vat>{number_pattern}).?(?P<vat_rate>{number_pattern}%).?(?P<total_vat>{number_pattern}).?(?P<discount>{number_pattern})?.?(?P<total_price_vat>{number_pattern})"
